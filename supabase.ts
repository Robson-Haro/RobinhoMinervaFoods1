...
export type ParseResult = { colunas: string[]; rows: Record<string, string>[]; total: number }
...
export async function parseArquivo(file: File): Promise<ParseResult> {
  ...
  if (ext === 'csv') {
    return new Promise((resolve, reject) => {
      Papa.parse(file, {
        ...
        complete: (r) => { const rows = r.data as Record<string, string>[]; resolve({ colunas: r.meta.fields || [], rows, total: rows.length }) },
        error: reject,
      })
    })
  }
  ...
  return { colunas: rows.length > 0 ? Object.keys(rows[0]) : [], rows, total: rows.length }
}

export function mapearCandidato(row: Record<string, string>, mapeamento: Record<string, string>): DadosCandidato {
  const get = (key: string) => (mapeamento[key] ? (row[mapeamento[key]] || '').trim() : '')
  return {
    nome: get('nome'), telefone: formatTel(get('tel')), email: get('email'),
    linkedin_url: get('linkedin'), cidade: get('cidade'), estado: get('estado'),
    cargo_atual: get('cargo'), empresa_atual: get('empresa'), experiencias: get('exp'),
    formacao: get('form'), idiomas: get('idiomas'), salario_pret: get('salario'),
    dados_brutos: row,
  }
}
...
export function autoMapear(colunas: string[]): Record<string, string> {
  ...
  for (const [campo, aliases] of Object.entries(AUTO_RULES)) {
    const match = colunas.find(col => aliases.some(a => col.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').includes(a)))
...
export function exportarCSV(candidatos: Record<string, unknown>[], filename: string) {
  ...
  const rows = candidatos.map((c, i) =>
    cols.map(k => { const v = k === 'rank' ? i + 1 : (c[k] ?? ''); return typeof v === 'string' && v.includes(',') ? `"${v}"` : v }).join(', ')
  )
